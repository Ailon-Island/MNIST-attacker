...
class ODI_PGD_ad_attacker(attacker):
    def __init__(self, ODI_k, k, epsilon, ODI_alpha, alpha, model, training=True, train_lambda=1., train_attack_on=False,
                 device='cpu'):
        super(ODI_PGD_ad_attacker, self).__init__(model, epsilon, training, train_lambda, train_attack_on, device)
        self.ODI_k = ODI_k
    ...
    def generate_perturbation(self, img_raw, target):
        ...
        for i in range(iter):
            ...
            perturbation = project(image.grad.data, iter_alpha)
