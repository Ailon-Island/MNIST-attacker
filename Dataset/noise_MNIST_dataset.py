...
class noise_classifier_MNIST(Dataset):
    def __init__(self, *attacker_kargs):
        ...
        try:
            FGSM_kwargs_in_file, PGD_kwargs_in_file, PGD_ad_kwargs_in_file, Deep_Fool_kwargs_in_file, self.noise_training_imgs = np.load('noise_classifier_MNIST_dataset.npy', allow_pickle=True)
            ...
            for batch_idx in range(num_seg):
                images.append(np.load('FGSM_seg_{}.npy'.format(batch_idx), allow_pickle=True))
            ...
            images = []
            for batch_idx in range(num_seg):
                images.append(images, np.load('PGD_seg_{}.npy'.format(batch_idx), allow_pickle=True))
            images = np.row_stack(images)
            ...
            images = []
            for batch_idx in range(num_seg):
                images.append(images, np.load('PGD_ad_seg_{}.npy'.format(batch_idx), allow_pickle=True))
            images = np.row_stack(images)
            ...
            images = []
            for batch_idx in range(num_seg):
                images.append(images, np.load('Deep_Fool_seg_{}.npy'.format(batch_idx), allow_pickle=True))
            images = np.row_stack(images)
